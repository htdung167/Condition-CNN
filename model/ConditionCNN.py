...
class ConditionTrain:
    ...
    def __init__(self,label, master_classes, sub_classes, art_classes, input_img_shape):
        ...
        c_1_pred = Dense(self.master_classes, activation='softmax', name='master_output')(c_1_bch) #Retrain
        # c_1_pred = Dense(self.master_classes, activation='softmax', name='master_output_shopee')(c_1_bch) 
...
class ConditionTrain_FinetuneFromOtherDataset:
    '''This model is based off of VGG16 with the addition of BatchNorm layers and then branching '''
    ...
    def __init__(self,label, master_classes, sub_classes, art_classes, input_img_shape):
        ...
        c_1_pred = Dense(self.master_classes, activation='softmax', name='master_output_2')(c_1_bch) 
        ...
        #--- masterCategory conditioning for subCategory branch ---
        c_1_condition = Dense(self.sub_classes, activation=None, use_bias=False, kernel_constraint=NonNegUnitNorm(),kernel_initializer=Zeros(),name='c_1_condition_2')(input_master)

        c_2_raw = Dense(self.sub_classes, activation='relu', name='c_2_raw_2')(c_2_bch)

        preds_features = Add()([c_1_condition,c_2_raw])

        c_2_pred = Softmax(name='sub_output_2')(preds_features)
        ...
        #--- subCategory conditioning  for articleType branch ---
        c_2_condition = Dense(self.art_classes, activation=None, use_bias=False, kernel_constraint=NonNegUnitNorm(),kernel_initializer=Zeros(),name='c_2_condition_2')(input_sub)

        # c_3_raw = Dense(self.art_classes, activation='relu', name='c_3_raw')(y)
        c_3_raw = Dense(self.art_classes, activation='relu', name='c_3_raw_2')(y)
        preds_features = Add()([c_2_condition,c_3_raw])
        fine_pred = Softmax(name='article_output_2')(preds_features)
        ...
        losses = {
            "master_output_2": "categorical_crossentropy",
            "sub_output_2": "categorical_crossentropy",
            "article_output_2": "categorical_crossentropy"
        }

        model.compile(optimizer=SGD(lr=0.001, momentum=0.9), loss=losses,
                      metrics=['categorical_accuracy'])

        checkpoint1 = ModelCheckpoint("./weights/"+label+"_best_weights.h5", monitor='val_loss', verbose=1,
            save_best_only=True, save_weights_only=True,mode='auto')

        checkpoin2 = ReduceLROnPlateau(monitor='val_loss', patience=5, verbose=1, factor=0.1, min_lr=0.000001)
        self.cbks = [checkpoint1, checkpoin2]
        self.model = model

class ConditionTest_FinetuneFromOtherDataset:
    '''One parameter model which is a keras model'''
    ...
    def __init__(self,label, master_classes, sub_classes, art_classes, input_img_shape):
        ...
        #--- masterCategory conditioning for subCategory branch ---
        c_1_condition = Dense(self.sub_classes, activation=None, use_bias=False, kernel_constraint=NonNegUnitNorm(),name='c_1_condition_2')(c_1_pred)
        c_2_raw = Dense(self.sub_classes, activation='relu', name='c_2_raw_2')(c_2_bch)
        preds_features = Add()([c_1_condition,c_2_raw])
        c_2_pred = Softmax(name='sub_output_2')(preds_features)
        ...
        #--- subCategory conditioning  for articleType branch ---
        c_2_condition = Dense(self.art_classes, activation=None, use_bias=False, kernel_constraint=NonNegUnitNorm(),name='c_2_condition_2')(c_2_pred)
        c_3_raw = Dense(self.art_classes, activation='relu', name='c_3_raw_2')(y)
        preds_features = Add()([c_2_condition,c_3_raw])
        fine_pred = Softmax(name='article_output_2')(preds_features)
